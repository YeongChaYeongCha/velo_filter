...
void EuclideanCluster::cloud_cluster_callback(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr){
	...
	for(std::vector<pcl::PointIndices>::const_iterator it=cluster_indices.begin(); it!=cluster_indices.end(); ++it){
		...
		float cluster_width=cluster_max_point.x - cluster_min_point.x;
		float cluster_height=cluster_max_point.y - cluster_min_point.y;
		float cluster_depth=cluster_max_point.z - cluster_min_point.z;
		...
		viewer.spin();
		*/
		if(((cluster_width<0.5 && cluster_height<0.5) && (cluster_depth>0.2 && cluster_depth<1.0)) && (cluster_centroid.x<7.0 && abs(cluster_centroid.y)<4.0)){
		//if((cluster_centroid.x<10.0 && cluster_centroid.x>0.0) && (abs(cluster_centroid.y)<4.0)){	
			...
			bounding_box.scale.x=cluster_width;
			bounding_box.scale.y=cluster_height;
			bounding_box.scale.z=cluster_depth;
